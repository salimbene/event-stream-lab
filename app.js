...
if (process.env.VCAP_SERVICES) {
  ...
  if (process.argv.length < 5) {
    console.log(
      "ERROR: It appears the application is running is running without VCAP_SERVICES but the arguments are incorrect for local mode."
    );
    console.log(
      "\nUsage:\n" +
        "node " +
        process.argv[1] +
        " <kafka_brokers_sasl> <api_key> <cert_location> [ -consumer | -producer ]\n"
    );
    process.exit(-1);
  }
  ...
  if (process.argv.length === 6) {
    if ("-consumer" === process.argv[5]) runProducer = false;
    if ("-producer" === process.argv[5]) runConsumer = false;
  }
}
...
if (
  ...
  !opts.hasOwnProperty("api_key") ||
  !opts.hasOwnProperty("calocation")
) {
  console.error(
    "Error - Failed to retrieve options. Check that app is bound to an Event Streams service or that command line options are correct."
  );
  process.exit(-1);
}
