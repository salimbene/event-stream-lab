...
exports.buildProducer = function (Kafka, producer_opts, topicName, shutdown) {
  ...
  producer.on("delivery-report", function (err, dr) {
    if (err) {
      ...
      console.log(
        "Message produced, partition: " + dr.partition + " offset: " + dr.offset
      );
    }
  });

  async function sendMessages(counter, topic, partition) {
    const quotes = await getQuotes();
    const quoteIndex = Math.floor(Math.random() * quotes.length);
    const { dialog: quote } = quotes[quoteIndex];

    var message = new Buffer(quote || `This is a generic message #${counter}`);
    var key = "Key" + counter;
    ...
    producer.getMetadata(
      ...
      function (err, metadata) {
        if (err) {
          ...
          console.log(
            "Producer obtained metadata: " + JSON.stringify(metadata)
          );
          ...
          if (topicsByName.length === 0) {
            console.error(
              "ERROR - Topic " + topicName + " does not exist. Exiting"
            );
            shutdown(-1);
          }
        }
